# AGENTS.md — DURA Project Intelligence

> This file governs how Codex operates on the DURA codebase.
...
DURA enforces conformance against prose + schema specs versioned under `standards/`. Authority order: if any conflict, the standards file wins; AGENTS.md is a summary, not the source of truth.
...
Permitted operations (Codex may do without asking):
...
Prohibited operations (Codex must NOT do):

- Modify `.env`, `.env.local`, or any secrets file
- Modify this `AGENTS.md` without explicit human instruction per change
...
Codex must flag any changes to these paths explicitly:
...
Before committing changes to high-risk surfaces, Codex must output:
