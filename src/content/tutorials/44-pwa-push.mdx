...
function renderNotes() {
  ...
  notes.forEach((note, index) => {
    ...
    remove.addEventListener("click", () => window.deleteNote(index));
    card.append(text, timestamp, remove);
...
Note text is inserted with `textContent`, so saved HTML-like input remains literal text. DOM event listeners attach deletion behavior without generating inline JavaScript. This prevents a note from becoming executable markup.
