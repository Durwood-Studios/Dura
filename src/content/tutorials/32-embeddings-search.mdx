...
A score of 1.0 means identical vector direction, 0.0 means orthogonal vectors, and -1.0 means opposite direction. These are geometric facts, not guarantees of semantic identity or relevance. Score distributions depend on the model and corpus; choose relevance thresholds using labeled queries. Cosine similarity is not a calibrated percentage probability of a match.
...
async function performSearch(query: string): Promise<void> {
  ...
  resultsContainer.replaceChildren(...results.map((result) => {
    ...
    card.style.borderLeft = `3px solid ${scoreToColor(result.score)}`;
    const heading = document.createElement("div");
    heading.className = "result-header";
    heading.textContent = `${result.doc_id} · cosine similarity ${result.score.toFixed(3)}`;
    const text = document.createElement("p");
    text.className = "result-text";
    text.append(highlightQuery(result.text, query));
    card.append(heading, text);
    return card;
  }));
}

function highlightQuery(text: string, query: string): DocumentFragment {
  const fragment = document.createDocumentFragment();
  const needle = query.trim().toLowerCase();
  let offset = 0;
  if (!needle) { fragment.append(document.createTextNode(text)); return fragment; }
  const lower = text.toLowerCase();
  while (offset < text.length) {
    const index = lower.indexOf(needle, offset);
    if (index < 0) { fragment.append(document.createTextNode(text.slice(offset))); break; }
