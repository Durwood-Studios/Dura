...
function renderChart(
  ...
  const maxVal = Math.max(1, ...points.map((p) => p.value)) * 1.1;
  ...
  ctx.beginPath();
  points.forEach((p, i) => {
    const x = ((p.timestamp - minTs) / span) * width;
...
<div class="dashboard-grid">
  ...
  <div class="panel" id="panel-disk">
    <h3>Disk usage <span class="badge" id="disk-current"></span></h3>
    <canvas id="chart-disk" width="600" height="200"></canvas>
  </div>
  <div class="panel" id="panel-requestRate">
    <h3>Requests per second <span class="badge" id="requestRate-current"></span></h3>
    <canvas id="chart-requestRate" width="600" height="200"></canvas>
  </div>
</div>
```

```typescript
const chartConfigs: Record<string, {width:number; height:number; color:string}> = Object.fromEntries(
  ["cpu", "memory", "disk", "requestRate"].map(name => [name, {width:600,height:200,color:"#60a5fa"}])
);
function getContext(name: string): CanvasRenderingContext2D {
  const canvas = document.getElementById(`chart-${name}`);
  if (!(canvas instanceof HTMLCanvasElement)) throw new Error(`Missing chart: ${name}`);
  const context = canvas.getContext("2d");
  if (!context) throw new Error("Canvas rendering is unavailable");
  return context;
}
function updateBadge(name: string, value: number): void {
  const badge = document.getElementById(`${name}-current`);
...
source.addEventListener("metric", (event) => {
  ...
  if (!arr || !Number.isFinite(point?.timestamp) || !Number.isFinite(point?.value)) return;
  arr.push(point);
