...
Create a new Node 22+ project and keep the named TypeScript snippets in their indicated files. Install the compiler with `npm install -D typescript @types/node`, and set `"type": "module"` in package.json. Use this configuration:

```json
{
  "compilerOptions": {"target":"ES2022","module":"NodeNext","moduleResolution":"NodeNext","strict":true,"types":["node"],"rootDir":"src","outDir":"build"},
  "include":["src/**/*.ts"]
}
```

Create `content/` and `layouts/` before running. This educational parser supports a subset of Markdown and frontmatter, not arbitrary YAML or executable MDX. Templates and source documents must be trusted local author content; variable interpolation is not an HTML sanitizer. The development server binds to loopback and serves only files inside the output directory.
...
export async function scanDirectory(baseDir: string): Promise<SourceFile[]> {
...
import { LayoutManager } from "./templates/layouts.js";
...
export async function build(config: SiteConfig): Promise<void> {
  ...
  const documents = contentFiles.map(file => ({file, ...parseFrontmatter(readFileSync(file.absolutePath, "utf8"))}));
  const pages = documents.map(({file, metadata}) => {
    const slug = file.relativePath.replace(/\\/g, "/").replace(/\.mdx?$/, "");
    return {slug, title: String(metadata.title ?? slug), date: String(metadata.date ?? ""), url: `/${slug}/`};
  });
  let filesWritten = 0;

  for (const {file, metadata, content} of documents) {
...
Add a watch mode that requests a full rebuild when source files are modified. Use the Node.js `fs.watch` API with debouncing to avoid redundant rebuilds.

```typescript
// src/watch.ts
import { watch } from "fs";

export function watchSource(sourceDir: string, onChanged: (path: string) => void): void {
...
export function startDevServer(outputDir: string, port: number = 3000): {notifyReload: () => void} {
  ...
  const server = createServer((req, res) => {
    ...
    try {
      ...
      if (child.startsWith("..") || isAbsolute(child) || !statSync(filePath).isFile()) throw new Error("Outside output");
    } catch {
...
Wrap everything in a CLI that supports `build`, `serve`, and `watch` commands. Read configuration from a `site.config.json` file. This example implements `build` and `serve`; serving includes full rebuilds when source files change.

```typescript
// src/cli.ts
import { readFileSync } from "node:fs";
import { build, type SiteConfig } from "./build.js";
...
Run `npx tsc && node build/cli.js serve` to start the development workflow. Create a Markdown file in the content directory and watch it appear in the browser immediately. The full pipeline — scan, parse, render, template, write, reload — completes in milliseconds for typical sites, making the feedback loop nearly instant.
