import { PHASE_S } from "@/lib/phase-s";
import type { PhaseSLesson } from "@/lib/phase-s/types";

export function PhaseS(): React.ReactElement {
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-10">
      <header className="flex flex-col gap-2">
        <p className="text-xs font-medium tracking-wide text-[var(--color-text-muted)] uppercase">
          Phase S · Security Engineering
        </p>
...
function LessonRow({ lesson }: { lesson: PhaseSLesson }): React.ReactElement {
  return (
    <li className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4">
      <div className="flex items-baseline justify-between gap-3">
        <h3 className="text-base font-semibold text-[var(--color-text-primary)]">
          <span className="font-mono text-xs text-[var(--color-text-muted)]">S{lesson.order}</span>
          {" — "}
          {lesson.title}
        </h3>
