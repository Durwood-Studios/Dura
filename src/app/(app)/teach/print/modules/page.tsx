...
export default async function ModuleIndexPage(): Promise<React.ReactElement> {
  ...
  return (
    <main className="mx-auto max-w-[780px] px-8 py-10 text-neutral-900">
      <PrintChrome />
      ...
      {tree.map((node) => (
        <section key={node.phase.id} className="mb-8 break-inside-avoid">
          <h2 className="text-lg font-semibold text-neutral-900">{node.phase.title}</h2>
                ...
                <li key={modulePair.module.id}>
                  {disabled ? (
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-2 text-sm text-neutral-900 underline decoration-neutral-300 underline-offset-4 hover:decoration-emerald-500"
                      target="_blank"
                    >
                      {modulePair.module.title}
