...
### New learner-control proposals (manual review required)

`022-conflict-safe-sync.sql` introduces owner-scoped deletion tombstones, conditional mutable-record synchronization, daily study time and activity evidence, plus explicit goal targets. It revokes old direct mutable-table writes so older clients cannot bypass conflict/tombstone rules. Deploy the matching frontend with the reviewed migration; without it, cloud synchronization reports an error while local learning remains available. This is a distinct proposal from the previously reviewed live reconciliation, which is not changed by this file. Test `tests/supabase/conflict-safe-sync.sql` on a disposable fixture, never production. Hosted execution has not been performed.
